...
If authorization expires, the server stops accepting requests and closes the socket with private close code `4401`. The PWA does not apply ordinary reconnect backoff to that code: it stops the socket, re-runs the HTTP session bootstrap, and either opens a fresh socket with the replacement opaque session or enters the unauthenticated state. For every other unexpected close or failed upgrade, including a control-plane restart, the PWA pauses reconnect, performs the same HTTP bootstrap, and resumes the existing backoff only after the fresh opaque cookie is accepted. One socket lifecycle permits only one bootstrap at a time; explicit refresh, logout, and stale timers cannot revive an older lifecycle.

### Same-origin HTTP API

The checked HTTP contract is [`protocol/http-v1.schema.json`](../protocol/http-v1.schema.json) with the shared fixture at [`tests/fixtures/http_api_v1.json`](../tests/fixtures/http_api_v1.json). `GET /api/v1/session`, behind trusted proxy identity, issues or refreshes the opaque `__Host-herdr_session` cookie and returns only `authenticated: true`, the subject as `operator.display_name`, the configured VAPID public key or null, and the startup-validated HTTPS upstream logout URL. It never returns OIDC tokens or other claims. `GET /api/v1/csrf` requires that opaque session, returns only `{token}`, and does not set another cookie.

`POST /auth/logout` requires trusted proxy identity, the exact Origin, the opaque session, and CSRF. It deletes the server-side session, expires `__Host-herdr_session`, and confirms the deployment's validated HTTPS upstream OIDC logout URL. The PWA stops its socket and enters a synchronous logging-out state before this best-effort local request. It always navigates to the URL already received in session bootstrap, even if local revocation fails, so a provider session cannot silently recreate access on a shared device. The URL is configuration, never a provider-specific constant in the PWA. Read-only GET routes do not require Origin. Every state-changing HTTP route requires the exact configured Origin and a current CSRF token.

Every browser WebSocket registers a revocation watcher for its opaque session. Logout closes all sockets for that session immediately. A periodic no-touch validity check enforces idle and absolute expiry even when the browser sends nothing; state and output broadcasts never refresh idle lifetime. Every inbound browser message is revalidated before handling, and no state, output, or action result is delivered after invalidation.

Web Push uses `/api/v1/push/subscriptions`: POST registers the current browser subscription and DELETE removes a bounded endpoint list only for the authenticated operator. The subject predicate applies to every deletion, so another subject's endpoint is never removed. POST `/api/v1/push/subscriptions/reconcile` checks only the caller-supplied local endpoint and returns `{subscribed}`; it never lists another device or returns endpoint/key material. POST `/api/v1/push/subscriptions/replace` accepts a bounded, duplicate-free `source_endpoints` list and one current subscription. In one transaction it removes every existing same-subject source candidate and upserts the current target, including at the device limit. Other subjects' source candidates are ignored. A target owned by another subject remains forbidden, while an absent target with no same-subject candidate returns the same generic missing response whether candidates are absent or belong to another subject. Repeating a committed replacement after a lost response succeeds only when the target is owned by the same subject and its payload matches exactly; the retry also cleans any remaining same-subject candidates. The PWA uses it for browser endpoint rotation and when the configured VAPID application-server key changes. POST `/api/v1/push/test` sends only a generic `test` event through the bounded push service. It returns `{enabled:false}` when VAPID is not configured.

Push fanout uses bounded workers and an independent timeout per device under the request-wide context. The server-wide deadline is derived from the 20-device limit, worker count, per-device timeout, and margin, so every supported device receives a complete independent attempt even when the first worker batch stalls. Retry behavior and 404/410 cleanup remain endpoint-local. Push endpoints must be HTTPS DNS hostnames on port 443, without userinfo, IP literals, or fragments. Delivery uses a proxy-free HTTP transport that never follows redirects, resolves and validates DNS again immediately before dialing, rejects non-public and special-purpose IPv4/IPv6 ranges, and dials only a validated IP while retaining TLS verification and SNI for the original hostname. There is no fixed push-provider hostname allowlist. At startup, the control plane validates the VAPID subscriber URI, both base64url key encodings, P-256 private scalar, derived public key, and exact key-pair match before advertising the public key or accepting subscriptions.
...
Push subscriptions are bound to the authenticated operator on the same-origin HTTP endpoints above. They are not protocol credentials and never appear in browser WebSocket frames. IndexedDB stores only an explicit `opted_out` boolean and a bounded ordered list of old endpoint strings awaiting cleanup. IndexedDB is cleanup metadata, not a synchronization mechanism, and it never stores historical `p256dh`, `auth`, or target subscription payloads. Web Locks are required to enable notifications and to synchronize subscriptions across pages, tabs, and the service worker. Browsers without Web Locks do not enable, test, reconcile, rotate, create, or register push subscriptions. They may inspect whether a local subscription already exists and offer an explicit opt-out action: the fallback first persists `opted_out: true`, durably appends the current endpoint when present, loads all pending sources, and submits them to subject-scoped bulk deletion. It may unsubscribe the local subscription only after its endpoint is durable, and it removes sources only after confirmed deletion. Failed deletion retains opt-out and every source for a later locked reconciliation; no-local cleanup is handled the same way. Before a locked VAPID-key rotation unsubscribes a local endpoint, it durably appends that endpoint; if the append fails or the list is full, it does not unsubscribe. The service worker appends a browser-changed old endpoint before session bootstrap, so authorization failure or temporarily disabled push cannot erase existing recovery metadata. Reconciliation sends the complete source list with the one current browser subscription and removes only the submitted endpoints from IndexedDB after confirmed success. An ambiguous transport failure therefore retains every candidate and retries idempotently. For A-to-B followed by B-to-C, the persisted list is `[A, B]` and the only key payload sent is C's current payload; the server replaces B if A-to-B committed, A if it did not, and accepts C idempotently if the response was lost. If a crash occurs after persisting and unsubscribing but before creating the replacement, reconciliation creates a subscription with the currently advertised VAPID key even in the normal non-creating page path, then performs the atomic replacement. Oversized, duplicate, or malformed persisted state fails explicitly instead of being truncated. Page reconciliations are serialized, and a newer queued VAPID key always runs after an older one. Locked opt-out persists `opted_out: true`, then submits the union of pending sources and the current endpoint to subject-scoped bulk deletion. Any reconciliation that observes opt-out retries cleanup and cannot create or register. Only an explicit user subscribe action clears opt-out before creating a new subscription; there is no automatic enable path. Unauthorized or push-disabled bootstraps perform no subscription mutation. If cleanup already removed every source and the target was never committed, replacement returns generic `404`; reconciliation retains the sources until registering the current subscription succeeds. This recovery runs inside the service worker and does not require reopening a page. It never recreates an old-key local subscription as rollback or deletes another operator's endpoint.

When the 16-source cleanup list is full and the current local endpoint is distinct, the local subscription is the durable retry handle because its endpoint cannot be appended. The no-Web-Locks opt-out submits one subject-scoped DELETE containing the 16 persisted sources plus the current endpoint. It unsubscribes locally and removes the 16 persisted sources only after confirmed deletion. On deletion failure it keeps `opted_out: true`, leaves the local subscription intact, and retries the same 17-endpoint cleanup without creating or registering a subscription.
...
5. Keep session, prompt, output, subscription key material, and pending-action state in memory only. The sole push exception is the bounded endpoint-only source list and opt-out boolean described above.
